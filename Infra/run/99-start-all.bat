...
call "%ROOT_DIR%\BE\gradlew.bat" -p "%ROOT_DIR%\BE" --parallel :services:auth-service:bootJar :services:intake-service:bootJar :services:biometric-service:bootJar :services:report-service:bootJar :services:notification-service:bootJar :services:gateway-service:bootJar
