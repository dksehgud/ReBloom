...
import {isValidNewPassword, normalizeFormValue,} from '../../features/counselor/utils/settingsValidation'
...
function CounselorSettingsPage() {
    ...
    const isPhoneValid = PHONE_NUMBER_PATTERN.test(profileForm.phone)
    const phoneError =
        profileForm.phone.length > 0 && !isPhoneValid
    ...
    const handleProfileSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!isPhoneValid) {
            setFeedback({
                title: '전화번호 형식을 확인해 주세요',
                message: '전화번호는 010-1234-5678 형식으로 입력해야 저장할 수 있어요.',
                tone: 'error',
            })
            return
        }
                    ...
                    {isProfileSection ? (
                        ...
                        >
                            ...
                            {profileFields.map((field) => (
                                <SettingsInput
                                    error={field.id === 'phone' ? phoneError : undefined}
                                    field={field}
